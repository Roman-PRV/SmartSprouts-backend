...
/**
 * End-to-end coverage for the addition_table game over the shared generic
 ...
 */
class AdditionTableApiTest extends TestCase
{
    use RefreshDatabase;

    private function additionGame(): Game
    {
    ...
    public function test_lists_all_levels_in_the_requested_locale(): void
    {
        ...
        $response->assertStatus(200)
            ...
            ->assertJsonFragment(['title' => 'Add 3']);

        // List shape comes from LevelDescriptionResource — no per-level operator.
        $this->assertArrayNotHasKey('operator', $response->json()[0]);
        $this->assertStringContainsString('icons/arithmetic/addition/1.png', $response->json()[0]['image_url']);
    }
    ...
    public function test_shows_a_single_level_with_operator_and_equations(): void
    {
        ...
        $response->assertStatus(200)
            ...
            ->assertJsonPath('equations.0', ['id' => 1, 'operand_a' => 3, 'operand_b' => 1, 'result' => 4])
            ->assertJsonPath('equations.9', ['id' => 10, 'operand_a' => 3, 'operand_b' => 10, 'result' => 13]);
    }
